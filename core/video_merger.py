...
class VideoMergerCore:
    ...
    Key Features:
        ...
        - Per-slide audio duration support
    ...
    Example Usage:
        ...
        # Create video from images with fixed duration
        ...
        # Create video with per-slide audio durations
        success = merger.create_video_with_audio_durations(
            input_dir=Path("images/"),
            output_path=Path("video.mp4"),
            audio_files=[Path("slide1.mp3"), Path("slide2.mp3")]
        )
    ...
    def get_audio_duration(self, audio_file: Path) -> float:
        ...
        try:
            ...
            self.progress_callback(f"Audio duration for {audio_file.name}: {duration:.2f}s")
            return duration

        except (subprocess.CalledProcessError, json.JSONDecodeError, KeyError, ValueError) as e:
            error_msg = f"Failed to get duration for {audio_file}: {e}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)

    def create_video_with_audio_durations(
        self,
        input_dir: Path,
        output_path: Path,
        ...
        """
        Create video from images where each slide duration matches its audio file duration.

        Args:
            input_dir: Directory containing input images (should be named sequentially)
            output_path: Path to output video file
            audio_files: List of audio files corresponding to each slide
        ...
        """
        try:
            self.progress_callback(f"Creating video with per-slide audio durations from: {input_dir}")
            ...
            # Validate that we have matching audio files
            if len(audio_files) != len(image_files):
                self.progress_callback(f"Warning: {len(image_files)} images but {len(audio_files)} audio files. Will use available audio files.")
            ...
            for i, audio_file in enumerate(audio_files):
                if audio_file.exists():
                    try:
                        ...
                        self.progress_callback(f"Warning: Could not get duration for {audio_file.name}, using 3.0s default")
                        audio_durations.append(3.0)
                else:
                    self.progress_callback(f"Warning: Audio file not found: {audio_file}, using 3.0s default")
                    audio_durations.append(3.0)

            self.progress_callback(f"Found {len(image_files)} image files with corresponding audio durations")

            # Create video from images with individual durations
            temp_video = output_path.parent / f"temp_{output_path.stem}.mp4"

            success = self._create_video_with_individual_durations(
                image_files, temp_video, audio_durations, fade_duration
            )

            if not success:
                return False

            # Concatenate all audio files for the final audio track
            if audio_files:
                self.progress_callback("Creating combined audio track...")
                combined_audio = output_path.parent / f"combined_audio_{output_path.stem}.mp3"
                success = self._concatenate_audio_files(audio_files, combined_audio)

                if success and combined_audio.exists():
                    # Add concatenated audio to video
                    self.progress_callback("Adding combined audio track to video...")
                    success = self._add_audio_to_video(temp_video, combined_audio, output_path)
                    ...
                    # Just rename temp video to final output if audio concat failed
                    self.progress_callback("Using video without audio due to audio processing failure")
                    temp_video.rename(output_path)
            else:
                # Just rename temp video to final output
                temp_video.rename(output_path)

            self.progress_callback("Video creation with audio durations completed successfully")
            return True

        except Exception as e:
            error_msg = f"Failed to create video with audio durations: {e}"
            logger.error(error_msg)
            self.progress_callback(f"Error: {error_msg}")
            return False

    def _concatenate_audio_files(self, audio_files: List[Path], output_path: Path) -> bool:
        """Concatenate multiple audio files into one."""
        try:
            # Create input file list for FFmpeg
            input_list_file = output_path.parent / "audio_concat_list.txt"
            ...
            self.progress_callback("Audio concatenation completed successfully")
            return True

        except subprocess.CalledProcessError as e:
            logger.error(f"FFmpeg error concatenating audio: {e.stderr}")
    ...
    def _create_video_with_individual_durations(
        self,
        ...
        """Create video from image files using individual durations for each slide."""
        try:
            # Create input file list for FFmpeg
            ...
            self.progress_callback("Running FFmpeg to create video with individual slide durations...")
            ...
            self.progress_callback("Video from images with individual durations created successfully")
            return True
